...
public class Dictionary implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -2374578295777792277L;

	/** A logger for this class */
	private static Redwood.RedwoodChannels log = Redwood.channels(Dictionary.class);

	public final Map<String, TagCount> dict = Generics.newHashMap();
	...
	public void fillWordTagCounts(Map<String, IntCounter<String>> wordTagCounts) {
	...
	/*
	 * public void release() { dict.clear(); }
	 * 
	 * public void addVPTaking(String verb, String tag, String partWord) { int h
	 * = verb.hashCode(); Integer i = Integer.valueOf(h); if
	 * (tag.startsWith("RP")) { if (this.partTakingVerbs.containsKey(i)) {
	 * this.partTakingVerbs.get(i).incPart(partWord); } else {
	 * this.partTakingVerbs.put(i, new CountWrapper(verb, 0, 0, 0, 0));
	 * this.partTakingVerbs.get(i).incPart(partWord); } } else if
	 * (tag.startsWith("RB")) { if (this.partTakingVerbs.containsKey(i)) {
	 * this.partTakingVerbs.get(i).incRB(partWord); } else {
	 * this.partTakingVerbs.put(i, new CountWrapper(verb, 0, 0, 0, 0));
	 * this.partTakingVerbs.get(i).incRB(partWord); } } else if
	 * (tag.startsWith("IN")) { if (this.partTakingVerbs.containsKey(i)) {
	 * this.partTakingVerbs.get(i).incIn(partWord); } else {
	 * this.partTakingVerbs.put(i, new CountWrapper(verb, 0, 0, 0, 0));
	...
	/*
	 * public void save(String filename) { try { DataOutputStream rf =
	 * IOUtils.getDataOutputStream(filename); save(rf); rf.close(); } catch
	 ...
	 * String[dict.keySet().size()]); try { int countAmbiguous = 0; int
	 * countUnAmbiguous = 0; int countAmbDisamb = 0; for (String word : arr) {
	 * if (word.indexOf('|') == -1) { continue; } TagCount count = get(word); if
	 * (count.numTags() > 1) { System.out.print(word); countAmbiguous++;
	 * tC.print(); System.out.println(); } else { String wordA =
	 * word.substring(0, word.indexOf('|')); if (get(wordA).numTags() > 1) {
	 * System.out.print(word); countAmbDisamb++; countUnAmbiguous++; tC.print();
	 * System.out.println(); } else { countUnAmbiguous++; } }// else }
	 * System.out.println(" ambg " + countAmbiguous + " unambg " +
	 * countUnAmbiguous + " disamb " + countAmbDisamb); } catch (Exception e) {
	 * e.printStackTrace(); } }
	 */

	/**
	 * This makes ambiguity classes from all words in the dictionary and
	 * remembers their classes in the TagCounts
	 */
	...
	public String toString() {
		String str = "dict = \n";
		str += Utility.toString(dict);
		str += "\n";
		str += "partTakingVerbs = \n";
		str += Utility.toString(partTakingVerbs);
		str += "\n";
	...
	public void writeToFile(String file) throws IOException {
		...
		for (Map.Entry<String, TagCount> entry : dict.entrySet()) {
			...
			System.out.println(entry.getKey() + " = " + Utility.toString(tags, ", ", "{}", tags.length));
		}

		ps.close();
		System.setOut(out);//设定新的输出流
	}
	...
	public void swap(String oldTag, String newTag) {
		for (Entry<String, TagCount> entry : dict.entrySet()) {
			if (entry.getValue().swap(oldTag, newTag)) {
	...
	public boolean equals(Object obj) {
		if (obj instanceof Dictionary) {
			...
			boolean ret = Utility.equals(dict, dic.dict);
			if (!ret)
				return false;
			return Utility.equals(partTakingVerbs, dic.partTakingVerbs);
		}
		return false;
	}
}
