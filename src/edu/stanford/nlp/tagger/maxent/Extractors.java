...
/**
 * Maintains a set of feature extractors for a maxent POS tagger and applies
 * them.
 *
...
public class Extractors implements Serializable {
	...
	transient List<Pair<Integer, Extractor>> local, // extractors only looking at current word
	...
	/**
	 * Find maximum left context of extractors. Used in TagInference to decide
	 * windows for dynamic programming.
	 * 
	...
	/**
	 * Find maximum right context of extractors. Used in TagInference to decide
	 * windows for dynamic programming.
	 * 
	 ...
	 * IOUtils.getDataOutputStream(filename); rf.writeInt(v.length); for
	 * (Extractor extr : v) { rf.writeBytes(extr.toString()); } rf.close(); }
	 * catch (IOException e) { e.printStackTrace(); } }
	 * 
	 * public void read(String filename) { try { InDataStreamFile rf = new
	 * InDataStreamFile(filename); int len = rf.readInt(); v = new
	 * Extractor[len]; //GlobalHolder.init(); } catch (IOException e) {
	 * e.printStackTrace(); } }
	 */

	public Extractor get(int index) {
		return v[index];
	}
	...
	/**
	 * Prints out the pair of {@code Extractors} objects found in the file that
