...
/**
 * Reads tagged data from a file and creates a dictionary. The tagged data has
 * to be whitespace-separated items, with the word and tag split off by a
 * delimiter character, which is found as the last instance of the delimiter
...
public class ReadDataTagged {
	...
	/**
	 * Frees the memory that is stored in this object by dropping the word-tag
	 * data.
	 */
	void release() {
		v.clear();
	...
	private void loadFile(TaggedFileReader reader, Map<String, IntCounter<String>> wordTagCounts) {
		...
		log.info("Read numElements = " + numElements + " Elements including EOS mark(.$$.)");
		log.info("numElements - numWords = " + (numElements - numWords));
	}

	/**
	 * Returns the number of tokens in the data read, which is the number of
	 * words plus one end sentence token per sentence.
	 * 
