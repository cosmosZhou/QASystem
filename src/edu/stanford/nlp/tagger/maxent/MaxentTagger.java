 ...
 *
 * You can tag things through the Java API or from the command line. The two
 * English taggers included in this distribution are:
 * <ul>
 ...
 * <li>A model using only left second-order sequence information and similar but
 * less unknown words and lexical features as the previous model in
 * {@code edu/stanford/nlp/models/pos-tagger/english-left3words/english-left3words-distsim.tagger}
 * This tagger runs a lot faster, and is recommended for general use. Its
 * accuracy was 96.92% on Penn Treebank WSJ secs. 22-24.</li>
 * </ul>
 *
 * <h3>Using the Java API</h3>
 * <dl>
 * <dt>A MaxentTagger can be made with a constructor taking as argument the
 * location of parameter files for a trained tagger:</dt>
 * <dd>
 * {@code MaxentTagger tagger = new MaxentTagger("models/left3words-wsj-0-18.tagger"); }</dd>
 * <p>
 * <dt>A default path is provided for the location of the tagger on the Stanford
 * NLP machines:</dt>
 ...
 * DEFAULT_NLP_GROUP_MODEL_PATH will instead point to the directory given in
 * NLP_DATA_HOME.</dt>
 * <p>
 * <dt>To tag a List of HasWord and get a List of TaggedWord, you can use one
 * of:</dt>
 ...
 * <dd>
 * {@code String taggedString = tagger.tagString("Here's a tagged string.")}</dd>
 * <p>
 * <dt>To tag a string of <i>correctly tokenized</i>, whitespace-separated words
 * and get a string of tagged words back:</dt>
 * <dd>
 * {@code String taggedString = tagger.tagTokenizedString("Here 's a tagged string .")}</dd>
 * </dl>
 * <p>
 * The {@code tagString} method uses the default tokenizer (PTBTokenizer). If
 * you wish to control tokenization, you may wish to call
 ...
 * <h3>Training from the command line</h3> To train a model from the command
 * line, first generate a property file:
 * 
 * <pre>
 ...
 * you can set in your trained model. You can modify the properties file, or use
 * the default options. To train, run:
 * 
 * <pre>
 ...
 *
 * Useful flags for controlling the amount of output are -verbose, which prints
 * extra debugging information, and -verboseResults, which prints full
 * information about intermediate results. -verbose defaults to false and
 * -verboseResults defaults to true.
 *
 ...
 * </pre>
 * 
 * You can use the same properties file as for training if you pass it in with
 * the "-props" argument. The most important arguments for tagging (besides
 * "model" and "file") are "tokenize" and "tokenizerFactory". See below for more
 * details. <br>
 * Note that the tagger assumes input has not yet been tokenized and by default
 * tokenizes it using a default English tokenizer. If your input has already
 * been tokenized, use the flag "-tokenize false".
 *
 * <p>
 * Parameters can be defined using a Properties file (specified on the
 * command-line with {@code -prop} <i>propFile</i>), or directly on the command
 * line (by preceding their name with a minus sign ("-") to turn them into a
 * flag. The following properties are recognized:
 * </p>
 * <table border="1">
 * <tr>
 ...
 * <td>N/A</td>
 * <td>All</td>
 * <td>Path and filename where you would like to save the model (training) or
 * where the model should be loaded from (testing, tagging).</td>
 * </tr>
 * <tr>
 * <td>trainFile</td>
 * <td>String</td>
 * <td>N/A</td>
 * <td>Train</td>
 * <td>Path to the file holding the training data; specifying this option puts
 * the tagger in training mode. Only one of 'trainFile','testFile','textFile',
 * and 'dump' may be specified.<br>
 * There are three formats possible. The first is a text file of tagged data.
 * Each line is considered a separate sentence. In each sentence, words are
 * separated by whitespace. Each word must have a tag, which is separated from
 * the token using the specified {@code tagSeparator}. This format, called TEXT,
 * is the default format.<br />
 * The second format is a file of Penn Treebank formatted tree files. Trees are
 * loaded one at a time and the tagged words in a tree are used as a training
 * sentence. To specify this format, preface the filename with
 * "{@code format=TREES,}". <br />
 * The final possible format is TSV files (tab-separated columns). To specify a
 * TSV file, set {@code trainFile} to
 * "{@code format=TSV,wordColumn=x,tagColumn=y,filename}". Column numbers are
 * indexed from 0, and sentences are separated with blank lines. The default
 * wordColumn is 0 and default tagColumn is 1. <br>
 * A file can be in a different character set encoding than the tagger's default
 * encoding by prefacing the filename with {@code "encoding=ENC"}. You can
 * specify the tagSeparator character in a TEXT file by prefacing the filename
 * with "tagSeparator=c". <br/>
 * Tree files can be fed through TreeTransformers and TreeNormalizers. To
 * specify a transformer, preface the filename with "treeTransformer=CLASSNAME".
 * To specify a normalizer, preface the filename with
 * "treeNormalizer=CLASSNAME". You can also filter trees using a
 * Filter&lt;Tree&gt;, which can be specified with "treeFilter=CLASSNAME". A
 * specific range of trees to be used can be specified with treeRange=X-Y.
 * Multiple parts of the range can be separated by : as opposed to the normal
 * separator of ,. For example, one could use the argument
 * "-treeRange=25-50:75-100". You can specify a TreeReaderFactory by prefacing
 * the filename with "trf=CLASSNAME". <br>
 * Multiple files can be specified by making a semicolon separated list of
 * files. Each file can have its own format specifiers as above.<br>
 * You will note that none of , ; or = can be in filenames.</td>
 * </tr>
 * <tr>
 * <td>testFile</td>
 * <td>String</td>
 * <td>N/A</td>
 * <td>Test</td>
 * <td>Path to the file holding the test data; specifying this option puts the
 * tagger in testing mode. Only one of 'trainFile','testFile','textFile', and
 * 'dump' may be specified. The same format as trainFile applies, but only one
 * file can be specified.</td>
 * </tr>
 * <tr>
 * <td>textFile</td>
 * <td>String</td>
 * <td>N/A</td>
 * <td>Tag</td>
 * <td>Path to the file holding the text to tag; specifying this option puts the
 * tagger in tagging mode. Only one of 'trainFile','testFile','textFile', and
 * 'dump' may be specified. No file reading options may be specified for
 * textFile</td>
 * </tr>
 * <tr>
 * <td>dump</td>
 ...
 * <td>N/A</td>
 * <td>Dump</td>
 * <td>Path to the file holding the model to dump; specifying this option puts
 * the tagger in dumping mode. Only one of 'trainFile','testFile','textFile',
 * and 'dump' may be specified.</td>
 * </tr>
 * <tr>
 * <td>genprops</td>
 * <td>boolean</td>
 * <td>N/A</td>
 * <td>N/A</td>
 * <td>Use this option to output a default properties file, containing
 * information about each of the possible configuration options.</td>
 * </tr>
 * <tr>
 * <td>tagSeparator</td>
 * <td>char</td>
 * <td>/</td>
 * <td>All</td>
 * <td>Separator character that separates word and part of speech tags, such as
 * out/IN or out_IN. For training and testing, this is the separator used in the
 * train/test files. For tagging, this is the character that will be inserted
 * between words and tags in the output.</td>
 * </tr>
 * <tr>
 * <td>encoding</td>
 * <td>String</td>
 * <td>UTF-8</td>
 * <td>All</td>
 * <td>Encoding of the read files (training, testing) and the output text
 * files.</td>
 * </tr>
 * <tr>
 * <td>tokenize</td>
 ...
 * <td>true</td>
 * <td>Tag,Test</td>
 * <td>Whether or not the file needs to be tokenized. If this is false, the
 * tagger assumes that white space separates words if and only if they should be
 * tagged as separate tokens, and that the input is strictly one sentence per
 * line.</td>
 * </tr>
 * <tr>
 * <td>tokenizerFactory</td>
 * <td>String</td>
 * <td>edu.stanford.nlp.<br>
 * process.PTBTokenizer</td>
 * <td>Tag,Test</td>
 * <td>Fully qualified class name of the tokenizer to use.
 * edu.stanford.nlp.process.PTBTokenizer does basic English tokenization.</td>
 * </tr>
 * <tr>
 * <td>tokenizerOptions</td>
 * <td>String</td>
 * <td></td>
 * <td>Tag,Test</td>
 * <td>Known options for the particular tokenizer used. A comma-separated list.
 * For PTBTokenizer, options of interest include {@code americanize=false} and
 * {@code asciiQuotes} (for German). Note that any choice of tokenizer options
 * that conflicts with the tokenization used in the tagger training data will
 * likely degrade tagger performance.</td>
 * </tr>
 * <tr>
 * <td>sentenceDelimiter</td>
 * <td>String</td>
 * <td>null</td>
 * <td>Tag,Test</td>
 * <td>A marker used to separate a text into sentences. If not set (equal to
 * {@code null}), sentence breaking is done by content (looking for periods,
 * etc.) Otherwise, it will break on this String, except that if the String is
 * "newline", it breaks on the String "\\n".</td>
 * </tr>
 * <tr>
 * <td>arch</td>
 * <td>String</td>
 * <td>generic</td>
 * <td>Train</td>
 * <td>Architecture of the model, as a comma-separated list of options, some
 * with a parenthesized integer argument written k here: this determines what
 * features are used to build your model. See {@link ExtractorFrames} and
 * {@link ExtractorFramesRare} for more information.</td>
 * </tr>
 * <tr>
 * <td>wordFunction</td>
 * <td>String</td>
 * <td>(none)</td>
 * <td>Train</td>
 * <td>A function to apply to the text before training or testing. Must inherit
 * from edu.stanford.nlp.util.Function&lt;String, String&gt;. Can be blank.</td>
 * </tr>
 * <tr>
 * <td>lang</td>
 * <td>String</td>
 * <td>english</td>
 * <td>Train</td>
 * <td>Language from which the part of speech tags are drawn. This option
 * determines which tags are considered closed-class (only fixed set of words
 * can be tagged with a closed-class tag, such as prepositions). Defined
 * languages are 'english' (Penn tag set), 'polish' (very rudimentary),
 * 'french', 'chinese', 'arabic', 'german', and 'medline'.</td>
 * </tr>
 * <tr>
 * <td>openClassTags</td>
 * <td>String</td>
 * <td>N/A</td>
 * <td>Train</td>
 * <td>Space separated list of tags that should be considered open-class. All
 * tags encountered that are not in this list are considered closed-class. E.g.
 * format: "NN VB"</td>
 * </tr>
 * <tr>
 * <td>closedClassTags</td>
 * <td>String</td>
 * <td>N/A</td>
 * <td>Train</td>
 * <td>Space separated list of tags that should be considered closed-class. All
 * tags encountered that are not in this list are considered open-class.</td>
 * </tr>
 * <tr>
 * <td>learnClosedClassTags</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Train</td>
 * <td>If true, induce which tags are closed-class by counting as closed-class
 * tags all those tags which have fewer unique word tokens than
 * closedClassTagThreshold.</td>
 * </tr>
 * <tr>
 * <td>closedClassTagThreshold</td>
 * <td>int</td>
 * <td>int</td>
 * <td>Train</td>
 * <td>Number of unique word tokens that a tag may have and still be considered
 * closed-class; relevant only if learnClosedClassTags is true.</td>
 * </tr>
 * <tr>
 * <td>sgml</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Tag, Test</td>
 * <td>Very basic tagging of the contents of all sgml fields; for more complex
 * mark-up, consider using the xmlInput option.</td>
 * </tr>
 * <tr>
 * <td>xmlInput</td>
 * <td>String</td>
 * <td></td>
 * <td>Tag, Test</td>
 * <td>Give a space separated list of tags in an XML file whose content you
 * would like tagged. Any internal tags that appear in the content of fields you
 * would like tagged will be discarded; the rest of the XML will be preserved
 * and the original text of specified fields will be replaced with the tagged
 * text.</td>
 * </tr>
 * <tr>
 * <td>outputFile</td>
 ...
 * <td>""</td>
 * <td>Tag</td>
 * <td>Path to write output to. If blank, stdout is used.</td>
 * </tr>
 * <tr>
 * <td>outputFormat</td>
 * <td>String</td>
 * <td>""</td>
 * <td>Tag</td>
 * <td>Output format. One of: slashTags (default), xml, or tsv</td>
 * </tr>
 * <tr>
 * <td>outputFormatOptions</td>
 * <td>String</td>
 * <td>""</td>
 * <td>Tag</td>
 ...
 * <td>""</td>
 * <td>Tag</td>
 * <td>Tags inside elements that match the regular expression given in the
 * String.</td>
 * </tr>
 * <tr>
 * <td>search</td>
 ...
 * <td>cg</td>
 * <td>Train</td>
 * <td>Specify the search method to be used in the optimization method for
 * training. Options are 'cg' (conjugate gradient), 'iis' (improved iterative
 * scaling), or 'qn' (quasi-newton).</td>
 * </tr>
 * <tr>
 * <td>sigmaSquared</td>
 * <td>double</td>
 * <td>0.5</td>
 * <td>Train</td>
 * <td>Sigma-squared smoothing/regularization parameter to be used for conjugate
 * gradient search. Default usually works reasonably well.</td>
 * </tr>
 * <tr>
 * <td>iterations</td>
 * <td>int</td>
 * <td>100</td>
 * <td>Train</td>
 * <td>Number of iterations to be used for improved iterative scaling.</td>
 * </tr>
 * <tr>
 * <td>rareWordThresh</td>
 * <td>int</td>
 * <td>5</td>
 * <td>Train</td>
 * <td>Words that appear fewer than this number of times during training are
 * considered rare words and use extra rare word features.</td>
 * </tr>
 * <tr>
 * <td>minFeatureThreshold</td>
 * <td>int</td>
 * <td>5</td>
 * <td>Train</td>
 * <td>Features whose history appears fewer than this number of times are
 * discarded.</td>
 * </tr>
 * <tr>
 * <td>curWordMinFeatureThreshold</td>
 * <td>int</td>
 * <td>2</td>
 * <td>Train</td>
 * <td>Words that occur more than this number of times will generate features
 * with all of the tags they've been seen with.</td>
 * </tr>
 * <tr>
 * <td>rareWordMinFeatureThresh</td>
 * <td>int</td>
 * <td>10</td>
 * <td>Train</td>
 * <td>Features of rare words whose histories occur fewer than this number of
 * times are discarded.</td>
 * </tr>
 * <tr>
 * <td>veryCommonWordThresh</td>
 * <td>int</td>
 * <td>250</td>
 * <td>Train</td>
 * <td>Words that occur more than this number of times form an equivalence class
 * by themselves. Ignored unless you are using ambiguity classes.</td>
 * </tr>
 * <tr>
 * <td>debug</td>
 * <td>boolean</td>
 * <td>boolean</td>
 * <td>All</td>
 * <td>Whether to write debugging information (words, top words, unknown words,
 * confusion matrix). Useful for error analysis.</td>
 * </tr>
 * <tr>
 * <td>debugPrefix</td>
 * <td>String</td>
 * <td>N/A</td>
 * <td>All</td>
 * <td>File (path) prefix for where to write out the debugging information
 * (relevant only if debug=true).</td>
 * </tr>
 * <tr>
 * <td>nthreads</td>
 * <td>int</td>
 * <td>1</td>
 * <td>Test,Text</td>
 * <td>Number of threads to use when processing text.</td>
 * </tr>
 * </table>
 * <p/>
 *
...
public class MaxentTagger extends Tagger implements ListProcessor<List<? extends HasWord>, List<TaggedWord>>, Serializable {
	 ...
	 * DEFAULT_NLP_GROUP_MODEL_PATH. Normally set to the location of the latest
	 * left3words tagger on the NLP machines, but can be changed by setting the
	 * environment variable NLP_DATA_HOME.
	 */
	 ...
	 * classpath resource, or URL. The tagger data is loaded when the
	 * constructor is called (this can be slow). This constructor first
	 * constructs a TaggerConfig object, which loads the tagger options from the
	 * modelFile.
	 *
	 * @param modelFile
	 *            Filename, classpath resource, or URL for the trained model
	 * @throws RuntimeIOException
	 *             if I/O errors or serialization errors
	 */
	 ...
	 *
	 * @param modelStream
	 *            The InputStream from which to read the model
	 * @throws RuntimeIOException
	 *             if I/O errors or serialization errors
	 */
	...
	/**
	 * Constructor for a tagger using a model stored in a particular file, with
	 * options taken from the supplied TaggerConfig. The tagger data is loaded
	 * when the constructor is called (this can be slow). This version assumes
	 * that the tagger options in the modelFile have already been loaded into
	 * the TaggerConfig (if that is desired).
	 *
	 * @param modelFile
	 *            Filename, classpath resource, or URL for the trained model
	 * @param config
	 ...
	 *
	 * @param modelFile
	 *            Where to initialize the tagger from. Most commonly, this is
	 *            the filename of the trained model, for example,
	 *            {@code /u/nlp/data/pos-tagger/wsj3t0-18-left3words/left3words-wsj-0-18.tagger}.
	 *            However, if it starts with "https?://" it will be interpreted
	 *            as a URL. One can also load models directly from the
	 *            classpath, as in loading from
	 *            {@code edu/stanford/nlp/models/pos-tagger/wsj3t0-18-bidirectional/bidirectional-distsim-wsj-0-18.tagger}.
	 * @param config
	 *            TaggerConfig based on command-line arguments
	 * @param printLoading
	 *            Whether to print a message saying what model file is being
	 *            loaded and how long it took when finished.
	 * @throws RuntimeIOException
	 *             if I/O errors or serialization errors
	 */
	 ...
	 *
	 * @param modelStream
	 *            An InputStream for reading the model file
	 * @param config
	 *            TaggerConfig based on command-line arguments
	 * @param printLoading
	 *            Whether to print a message saying what model file is being
	...
	public final Dictionary dict = new Dictionary();
	public TTags tags;
	...
	public List<Map<String, int[]>> fAssociations = Generics.newArrayList();
	//PairsHolder pairs = new PairsHolder();
	public Extractors extractors;
	public Extractors extractorsRare;
	public AmbiguityClasses ambClasses;
	public final boolean alltags = false;
	public final Map<String, Set<String>> tagTokens = Generics.newHashMap();
	...
	protected double[] defaultScores; // = null;
	...
	/**
	 * Determines which words are considered rare. All words with count in the
	 * training data strictly less than this number (standardly, &lt; 5) are
	 * considered rare.
	 */
	protected int rareWordThresh = RARE_WORD_THRESH;

	/**
	 * Determines which features are included in the model. The model includes
	 * features that occurred strictly more times than this number (standardly,
	 * &gt; 5) in the training data. Here I look only at the history (not the
	 * tag), so the history appearing this often is enough.
	 */
	public int minFeatureThresh = MIN_FEATURE_THRESH;

	/**
	 * This is a special threshold for the current word feature. Only words that
	 * have occurred strictly &gt; this number of times in total will generate
	 * word features with all of their occurring tags. The traditional default
	 * was 2.
	 */
	public int curWordMinFeatureThresh = CUR_WORD_MIN_FEATURE_THRESH;

	/**
	 * Determines which rare word features are included in the model. The
	 * features for rare words have a strictly higher support than this number
	 * are included. Traditional default is 10.
	 */
	public int rareWordMinFeatureThresh = RARE_WORD_MIN_FEATURE_THRESH;

	/**
	 * If using tag equivalence classes on following words, words that occur
	 * strictly more than this number of times (in total with any tag) are
	 * sufficiently frequent to form an equivalence class by themselves. (Not
	 * used unless using equivalence classes.)
	 *
	 * There are places in the code (ExtractorAmbiguityClass.java, for one) that
	 * assume this value is constant over the life of a tagger.
	 */
	public int veryCommonWordThresh = VERY_COMMON_WORD_THRESH;
	...
	/**
	 * This is a function used to preprocess all text before applying the tagger
	 * to it. For example, it could be a function to lowercase text, such as
	 * edu.stanford.nlp.util.LowercaseFunction (which makes the tagger case
	 * insensitive). It is applied in ReadDataTagged, which loads in the
	 * training data, and in TestSentence, which processes sentences for new
	 * queries. If any other classes are added or modified which use raw text,
	 * they must also use this function to keep results consistent. <br>
	 * An alternate design would have been to use the function at a lower level,
	 * such as at the extractor level. That would have require more invasive
	 * changes to the tagger, though, because other data structures such as the
	 * Dictionary would then be using raw text as well. This is also more
	 * efficient, in that the function is applied once at the start of the
	 * process.
	 */
	...
	protected void init(TaggerConfig config) {
		if (initted)
			return; // TODO: why not reinit?
	...
	/**
	 * Figures out what tokenizer factory might be described by the config. If
	 * it's described by name in the config, uses reflection to get the factory
	 * (which may cause an exception, of course...)
	 */
	...
	protected void setExtractorsGlobal() {
		extractors.setGlobalHolder(this);
		extractorsRare.setGlobalHolder(this);
	}

	/**
	 * Removes features that never have a non-zero weight for any tag from the
	 * fAssociations' appropriate Map.
	 */
	protected void removeDeadRules() {
	...
	/**
	 * Searching the lambda array for 0 entries, removes them. This saves a
	 * large chunk of space in the tagger models which are build with L1
	 * regularization. <br>
	 * After removing the zeros, go through the feature arrays and reindex the
	 * pointers into the lambda array. This saves some time later on at runtime.
	 */
	protected void simplifyLambda() {
	...
	/**
	 * This reads the complete tagger from a single model stored in a file, at a
	 * URL, or as a resource in a jar file, and initializes the tagger using a
	 * combination of the properties passed in and parameters from the file.
	 * <p>
	 * <i>Note for the future:</i> This assumes that the TaggerConfig in the
	 * file has already been read and used. This work is done inside the
	 * constructor of TaggerConfig. It might be better to refactor things so
	 * that is all done inside this method, but for the moment it seemed better
	 * to leave working code alone [cdm 2008].
	 *
	 * @param config
	 *            The tagger config
	 * @param modelFileOrUrl
	...
	/**
	 * This reads the complete tagger from a single model provided as an
	 * InputStream, and initializes the tagger using a combination of the
	 * properties passed in and parameters from the file.
	 * <p>
	 * <i>Note for the future:</i> This assumes that the TaggerConfig in the
	 * file has already been read and used. This work is done inside the
	 * constructor of TaggerConfig. It might be better to refactor things so
	 ...
	 *            The tagger config
	 * @param modelStream
	 *            The model provided as an InputStream
	 * @param printLoading
	 *            Whether to print a message saying what model file is being
	 *            loaded and how long it took when finished.
	 * @throws RuntimeIOException
	...
	/**
	 * This reads the complete tagger from a single model file, and inits the
	 * tagger using a combination of the properties passed in and parameters
	 * from the file.
	 * <p>
	 * <i>Note for the future: This assumes that the TaggerConfig in the file
	 * has already been read and used. It might be better to refactor things so
	 * that is all done inside this method, but for the moment it seemed better
	 * to leave working code alone [cdm 2008].</i>
	 *
	 * @param config
	 *            The tagger config
	 * @param rf
	 *            DataInputStream to read from. It's the caller's job to open
	...
	/**
	 * Tags the tokenized input string and returns the tagged version. This
	 * method requires the input to already be tokenized. The tagger wants input
	 * that is whitespace separated tokens, tokenized according to the
	 * conventions of the training data. (For instance, for the Penn Treebank,
	 * punctuation marks and possessive "'s" should be separated from words.)
	 *
	 * @param toTag
	...
	/**
	 * Tags the input string and returns the tagged version. This method
	 * tokenizes the input into words in perhaps multiple sentences and then
	 * tags those sentences. The default (PTB English) tokenizer is used.
	 *
	 * @param toTag
	 *            The untagged input String
	...
	/**
	 * Tags the Words in each Sentence in the given List with their grammatical
	 * part-of-speech. The returned List contains Sentences consisting of
	 * TaggedWords.
	 * <p>
	 * <b>NOTE: </b>The input document must contain sentences as its elements,
	 ...
	 * Returns a new Sentence that is a copy of the given sentence with all the
	 * words tagged with their part-of-speech. Convenience method when you only
	 * want to tag a single List instead of a List of Lists. If you supply
	 * tagSentence with a List of HasTag, and set reuseTags to true, the tagger
	 * will reuse the supplied tags.
	 *
	 * @param sentence
	 *            sentence to tag
	 * @param reuseTags
	 *            whether or not to reuse the given tag
	 * @return tagged sentence
	 */
	...
	/**
	 * Takes a sentence composed of CoreLabels and add the tags to the
	 * CoreLabels, modifying the input sentence. If reuseTags is set to true,
	 * any tags supplied with the CoreLabels are taken as correct.
	 */
	...
	/**
	 * Adds lemmas to the given list of CoreLabels, using the given Morphology
	 * object. The input list must already have tags set.
	 */
	...
	/**
	 * Casts a list of HasWords, which we secretly know to be CoreLabels, to a
	 * list of CoreLabels. Barfs if you didn't actually give it CoreLabels.
	 */
	 ...
	 * tokenizer, and returns a List of Sentence objects, which can then be fed
	 * into tagSentence.
	 *
	 * @param r
	 *            Reader where untokenized text is read
	...
	/**
	 * Reads data from r, tokenizes it with the given tokenizer, and returns a
	 * List of Lists of (extends) HasWord objects, which can then be fed into
	 * tagSentence.
	 *
	 * @param r
	 *            Reader where untokenized text is read
	 * @param tokenizerFactory
	 *            Tokenizer. This can be <code>null</code> in which case the
	 *            default English tokenizer (PTBTokenizerFactory) is used.
	 * @return List of tokenized sentences
	 */
	 ...
	 *
	 * @param config
	 *            Properties giving parameters for the testing run
	 */
	 ...
	 *
	 * @param config
	 *            Properties giving parameters for the training run
	 */
	 ...
	 *            A tagged sentence
	 * @param sentNum
	 *            The sentence index for XML printout
	 * @param outputLemmas
	 *            Whether to write the lemmas of words
	 */
	 ...
	 * Tags all of the text between xmlTags.
	 *
	 * The difference between using this and using runTagger in XML mode is that
	 * this preserves the XML structure outside of the list of elements to tag,
	 * whereas the runTagger method throws away all of the surrounding structure
	 * and returns tagged plain text.
	 */
	 ...
	 */
	private static void runTagger(TaggerConfig config) throws IOException, ClassNotFoundException, NoSuchMethodException, IllegalAccessException, java.lang.reflect.InvocationTargetException {
		if (config.getVerbose()) {
			Date now = new Date();
	...
	/**
	 * Runs the tagger when we're in TAG mode. In this mode, the config contains
	 * either the name of the file to tag or stdin. That file or input is then
	 * tagged.
	 */
	...
	public void runTaggerSGML(BufferedReader reader, BufferedWriter writer, OutputStyle outputStyle) throws IOException {
		Timing t = new Timing();
	 ...
	 *
	 * It takes input from the given {@code reader}, applies the tagger to it
	 * one sentence at a time (determined using documentPreprocessor), and
	 * writes the output to the given {@code writer}.
	 *
	 * The document is broken into sentences using the sentence processor
	 * determined in the tagger's TaggerConfig.
	 *
	 * {@code tagInside} makes the tagger run in XML mode.... If set to
	 * non-empty, instead of processing the document as one large text blob, it
	 * considers each region in between the given tag to be a separate text
	 * blob.
	 */
	...
	public void tagAndOutputSentence(List<? extends HasWord> sentence, boolean outputLemmas, Morphology morpha, OutputStyle outputStyle, boolean outputVerbosity, int numSentences, String separator, Writer writer) {
	...
	/**
	 * Command-line tagger interface. Can be used to train or test taggers, or
	 * to tag text, taking input from stdin or a file. See class documentation
	 * for usage.
	 *
	 * @param args
	...
	public String toString() {
		...
		for (int i = 0; i < extractors.size(); ++i) {
			str += extractors.get(i);
			str += Utility.lineSeparator;
			str += Utility.toString(fAssociations.get(i).keySet());
			str += Utility.lineSeparator;
		}

		for (int i = 0; i < extractorsRare.size(); ++i) {
			str += extractorsRare.get(i);
			str += Utility.lineSeparator;
			str += Utility.toString(fAssociations.get(extractors.size() + i).keySet());
			str += Utility.lineSeparator;
		}
		...
		str += "tagTokens = ";
		str += Utility.lineSeparator;
		str += Utility.toString(tagTokens, Utility.lineSeparator);

		return str;
	}

	protected void shuffle() {
		int indices[] = new int[prob.lambda.length];
		double lambda[] = new double[prob.lambda.length];
		...
		for (int i = 0; i < fAssociations.size(); ++i) {
			for (Map.Entry<String, int[]> entry : fAssociations.get(i).entrySet()) {
				int array[] = entry.getValue();
				for (int j = 0; j < array.length; ++j) {
					if (array[j] >= 0) {
						lambda[indices[index]] = prob.lambda[array[j]];
						array[j] = indices[index];
						++index;
					}
				}
			}
