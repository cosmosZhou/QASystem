...
/**
 * This class contains the basic feature extractors used for all words and tag
 * sequences (and interaction terms) for the MaxentTagger, but not the feature
 * extractors explicitly targeting generalization for rare or unknown words. The
 ...
 * <td>words</td>
 * <td>begin, end</td>
 * <td>Individual features for words begin ... end. If just one argument
 * words(-2) is given, then end is taken as 0. If begin is not less than or
 * equal to end, no features are made.</td>
 * </tr>
 * <tr>
 * <td>tags</td>
 * <td>begin, end</td>
 * <td>Individual features for tags begin ... end</td>
 * </tr>
 * <tr>
 * <td>biword</td>
 * <td>w1, w2</td>
 * <td>One feature for the pair of words w1, w2</td>
 * </tr>
 * <tr>
 * <td>biwords</td>
 * <td>begin, end</td>
 * <td>One feature for each sequential pair of words from begin to end</td>
 * </tr>
 * <tr>
 * <td>twoTags</td>
 * <td>t1, t2</td>
 * <td>One feature for the pair of tags t1, t2</td>
 * </tr>
 * <tr>
 * <td>lowercasewords</td>
 * <td>begin, end</td>
 * <td>One feature for each word begin ... end, lowercased</td>
 * </tr>
 * <tr>
 * <td>order</td>
 * <td>left, right</td>
 * <td>A feature for tags left through 0 and a feature for tags 0 through right.
 * Lower order left and right features are also added. This gets very expensive
 * for higher order terms.</td>
 * </tr>
 * <tr>
 * <td>wordTag</td>
 * <td>w, t</td>
 * <td>A feature combining word w and tag t.</td>
 * </tr>
 * <tr>
 * <td>wordTwoTags</td>
 * <td>w, t1, t2</td>
 * <td>A feature combining word w and tags t1, t2.</td>
 * </tr>
 * <tr>
 * <td>threeTags</td>
 * <td>t1, t2, t3</td>
 * <td>A feature combining tags t1, t2, t3.</td>
 * </tr>
 * <tr>
 * <td>vbn</td>
 * <td>length</td>
 * <td>A feature that looks at the left length words for something that appears
 * to be a VBN (in English) without looking at the actual tags. It is zeroeth
 * order, as it does not look at the tag predictions. It also is never used,
 * since it doesn't seem to help.</td>
 * </tr>
 * <tr>
 * <td>allwordshapes</td>
 * <td>left, right</td>
 * <td>Word shape features, eg transform Foo5 into Xxx# (not exactly like that,
 * but that general idea). Creates individual features for each word left ...
 * right. Compare with the feature "wordshapes" in ExtractorFramesRare, which is
 * only applied to rare words. Fairly English-specific. Slightly increases
 * accuracy.</td>
 * </tr>
 * <tr>
 * <td>allunicodeshapes</td>
 * <td>left, right</td>
 * <td>Same thing, but works for unicode characters more generally.</td>
 * </tr>
 * <tr>
 * <td>allunicodeshapeconjunction</td>
 * <td>left, right</td>
 * <td>Instead of individual word shape features, combines several word shapes
 * into one feature.</td>
 * </tr>
 * </table>
 *
 * See {@link ExtractorFramesRare} for more options. <br>
 * There are also macro features: <br>
 * left3words = words(-1,1),order(2) <br>
 * left5words = words(-2,2),order(2) <br>
 * generic = words(-1,1),order(2),biwords(-1,0),wordTag(0,-1) <br>
 * bidirectional5words = words(-2,2),order(-2,2),twoTags(-1,1),
 * wordTag(0,-1),wordTag(0,1),biwords(-1,1) <br>
 * bidirectional = words(-1,1),order(-2,2),twoTags(-1,1),
 * wordTag(0,-1),wordTag(0,1),biwords(-1,1) <br>
 * german = some random stuff <br>
 * sighan2005 = some other random stuff <br>
 * The left3words architectures are faster, but slightly less accurate, than the
 * bidirectional architectures. 'naacl2003unknowns' was our traditional set of
 * unknown word features, but you can now specify features more flexibility via
 * the various other supported keywords. <br>
 * 
...
public class ExtractorFrames {
	...
	public static final Extractor cWord = new Extractor(0, false);
	...
	private static final Extractor[] eFrames_german = { cWord, prevWord, nextWord, nextTag, prevTag, new ExtractorContinuousTagConjunction(-2), prevTagWord, new ExtractorTwoWords(-1, 0) };
	...
	public static class ExtractorWordTag extends Extractor {
		...
		public String toString() {
			return (getClass().getName() + "(w" + wordPosition + ",t" + position + ')');
		}
	}

	/**
	 * The word in lower-cased version. Always uses Locale.ENGLISH.
	 */
	...
	/**
	 * The current word if it is capitalized, zero otherwise. Always uses
	 * Locale.ENGLISH.
	 */
	...
	/**
	 * This extractor extracts two words in conjunction. The one argument
	 * constructor gives you leftPosition and leftPosition+1, but with the two
	 * argument constructor, they can be any pair of word positions.
	 */
	public static class ExtractorTwoWords extends Extractor {
		...
		@Override
		public String toString() {
			return (getClass().getName() + "(w" + leftPosition + ",w" + rightPosition + ')');
		}
	}

	/**
	 * This extractor extracts two tags in conjunction. The one argument
	 * constructor gives you leftPosition and leftPosition+1, but with the two
	 * argument constructor, they can be any pair of tag positions.
	 */
	public static class ExtractorTwoTags extends Extractor {
		...
		@Override
		public String toString() {
			return (getClass().getName() + "(t" + leftPosition + ",t" + rightPosition + ')');
		}
	}
	 ...
	 */
	public static class ExtractorTwoWordsTag extends Extractor {
		...
		@Override
		public String toString() {
			return (getClass().getName() + "(w" + leftWord + ",t" + tag + ",w" + rightWord + ')');
		}
	}

	/**
	 * This extractor extracts several contiguous tags only on one side of
	 * position 0. E.g., use constructor argument -3 for an order 3 predictor on
	 * the left. isLocal=false, isDynamic=true (through super call)
	 */
	...
	public static class ExtractorThreeTags extends Extractor {
		...
		@Override
		public String toString() {
			return (getClass().getName() + "(t" + position1 + ",t" + position2 + ",t" + position3 + ')');
		}
	}
	...
	public static class ExtractorWordTwoTags extends Extractor {
		...
		@Override
		public String toString() {
			return (getClass().getName() + "(t" + position1 + ",t" + position2 + ",w" + word + ')');
		}
	}

	public static class ExtractorIntonation extends Extractor {
		public ExtractorIntonation() {
			super(0, false);
		}
		...
		transient public Map<String, TagCount> dict;
		...
		String extract(History h, PairsHolder pH) {
			...
			if (word.charAt(0) == '几') {
				...
				for (;;) {
					intonation = pH.getWord(h, ++position);
					switch (intonation) {
					case "的":
						String next = pH.getWord(h, position + 1);
						if (next.equals("NA"))
							break;
						if (",.;!?)]}，。：；！？、…．”】》）＇".indexOf(next.substring(0, 1)) >= 0)
							break;
						continue;
					case "NA":
						break;
			...
			case "啥":
				for (;;) {
					intonation = pH.getWord(h, ++position);
					if (!intonation.equals("NA"))
						intonation = intonation.substring(0, 1);
					switch (intonation) {
					case "NA":
						break;
					case "，":
						break;
					case ",":
						if (pH.getWord(h, position + 1).substring(0, 1).equals("都"))
							intonation = ",都";
						else
							intonation = ",";
						break;
			...
			case "在":
				for (;;) {
					...
					TagCount tags = dict.get(intonation);
					if (tags == null) {
						continue;
					}

					if (tags.map.containsKey("PU")) {
						intonation = "PU";
						break;
					}
					if (tags.map.containsKey("VT") || tags.map.containsKey("VI") || tags.map.containsKey("VA")) {
						intonation = "VT";
						break;
					}
					if (tags.map.containsKey("MD")) {
						intonation = "MD";
						break;
					}
				}

				return "AT!" + intonation;
			case "多少":
				for (;;) {
					intonation = pH.getWord(h, --position);
					switch (intonation) {
...
/**
 * Extracts a boolean indicating whether the given word is preceded by an
 * auxiliary verb.
 */
...
/**
 * Extracts a boolean indicating whether the given word is preceded by a
 * semi-auxiliary verb.
 */
