 ...
 * {@code List<IN> classifyWithGlobalInformation(List<IN> tokenSequence, final CoreMap document, final CoreMap sentence); }
 * <br>
 * {@code void train(Collection<List<IN>> docs, DocumentReaderAndWriter<IN> readerAndWriter); }
 * <br>
 ...
 * first, and throw UnsupportedOperationException for the rest. Additionally,
 * this method throws UnsupportedOperationException by default, but is
 * implemented for some classifiers: <br>
 * {@code Pair<Counter<Integer>, TwoDimensionalCounter<Integer,String>> printProbsDocument(List<CoreLabel> document); }
 * <br>
 *
 * @author Jenny Finkel
 * @author Dan Klein
...
public abstract class AbstractSequenceClassifier<IN extends CoreMap> implements Function<String, String> {
	...
	/**
	 * Different threads can add or query knownLCWords at the same time, so we
	 * need a concurrent data structure. Created in reinit().
	 */
	protected MaxSizeConcurrentHashSet<String> knownLCWords; // = null;

	protected DocumentReaderAndWriter<IN> defaultReaderAndWriter;

	/**
	 * This is the DocumentReaderAndWriter used for reading training and testing
	 * files. It is the DocumentReaderAndWriter specified by the readerAndWriter
	 * flag and defaults to
	 * {@code edu.stanford.nlp.sequences.ColumnDocumentReaderAndWriter} which is
	 * suitable for reading CoNLL-style TSV files.
	 *
	...
	protected DocumentReaderAndWriter<IN> plainTextReaderAndWriter;

	/**
	 * This is the default DocumentReaderAndWriter used for reading text files
	 * for runtime classification. It is the DocumentReaderAndWriter specified
	 * by the plainTextDocumentReaderAndWriter flag and defaults to
	 * {@code edu.stanford.nlp.sequences.PlainTextDocumentReaderAndWriter} which
	 * is suitable for reading plain text files, in languages with a Tokenizer
	 * available.
	 *
	...
	/**
	 * Takes a list of tokens and provides the K best sequence labelings of
	 * these tokens with their scores.
	 *
	 * @param doc
	 *            The List of tokens
	 * @param answerField
	 *            The key for each token into which the label for the token will
	 *            be written
	 * @param k
	 *            The number of best sequence labelings to generate
	 * @return A Counter where each key is a List of tokens with labels written
	 *         in the answerField and its value is the score (conditional
	 *         probability) assigned to this labeling of the sequence.
	 */
	 ...
	 *
	 * @param documents
	 *            The documents in an ObjectBank to classify.
	 * @return {@link List} of classified sentences (each a List of something
	 *         that extends {@link CoreMap}).
	 */
	 ...
	 * (possibly empty, but not {@code null}) List of Triples. Each Triple is an
	 * entity name, followed by beginning and ending character offsets in the
	 * original String. Character offsets can be thought of as fenceposts
	...
	/**
	 * Have a word segmenter segment a String into a list of words. ONLY USE IF
	 * YOU LOADED A CHINESE WORD SEGMENTER!!!!!
	 *
	 * @param sentence
	 *            The string to be classified
	...
	public String[] segmentStringToArray(String sentence) {
		return segmentStringToArray(sentence, defaultReaderAndWriter);
	}

	public String[] segmentStringToArray(String sentence, DocumentReaderAndWriter<IN> readerAndWriter) {
	...
	public List<String> segmentString(String sentence, DocumentReaderAndWriter<IN> readerAndWriter) {
		return Arrays.asList(segmentStringToArray(sentence, readerAndWriter));
	}
	 ...
	 * the reading method specified in {@code flags.documentReader}, and for
	 * some reader choices, the column mapping given in {@code flags.map}.
	 *
	 * @param in
	...
	/**
	 * Takes the files, reads them in, and prints out the likelihood of each
	 * possible label at each point.
	 *
	 * @param testFiles
	 ...
	 * flags.plainTextDocumentReaderAndWriter to determine how to read the
	 * textFile format. By default this gives
	 * edu.stanford.nlp.sequences.PlainTextDocumentReaderAndWriter. <i>Note:</i>
	 * This means that it works right for a plain textFile (and not a
	 * tab-separated columns test file).
	 *
	 * @param textFile
	 *            The file to test on.
	 */
	 ...
	 * stdout (with timing to stderr). This uses the value of
	 * flags.documentReader to determine testFile format. By default, this means
	 * that it is set up to read a tab-separated columns test file
	 *
	 * @param testFile
	 *            The file to test on.
	 * @param outputScores
	 *            Whether to calculate and then log performance scores (P/R/F1)
	...
	/**
	 * If the flag {@code outputEncoding} is defined, the output is written in
	 * that character encoding, otherwise in the system default character
	 * encoding.
	 */
	...
	/**
	 * Run the classifier on a collection of text files. Uses the
	 * plainTextReaderAndWriter to process them.
	 *
	 * @param textFiles
	 *            A File Collection to process.
	 * @throws IOException
	 *             For any IO error
	 */
	...
	public Triple<Double, Double, Double> classifyAndWriteAnswers(Collection<List<IN>> documents, DocumentReaderAndWriter<IN> readerWriter, boolean outputScores) throws IOException {
		return classifyAndWriteAnswers(documents, IOUtils.encodedOutputStreamPrintWriter(System.out, flags.outputEncoding, true), readerWriter, outputScores);
	}
	 ...
	 * @param readerWriter
	 * @param outputScores
	 *            Whether to calculate and output the performance scores
	 *            (P/R/F1) of the classifier
	 * @return A Triple of overall P/R/F1, if outputScores is true, else
	 *         {@code null}. The scores are done on a 0-100 scale like
	 *         percentages.
	 * @throws IOException
	 */
	 ...
	 *
	 * @param testFile
	 *            The name of the file to test on.
	 * @param k
	 *            How many best to print
	 * @param readerAndWriter
	 *            Class to be used for printing answers
	 */
	...
	/**
	 * Loads a classifier from the file, classpath resource, or URL specified by
	 * loadPath. If loadPath ends in .gz, uses a GZIPInputStream.
	 */
	 ...
	 *            can pass in {@code null} to override nothing.
	 */
