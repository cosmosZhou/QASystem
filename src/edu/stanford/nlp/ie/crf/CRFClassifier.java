...
/**
 * Class for sequence classification using a Conditional Random Field model. The
 * code has functionality for different document formats, but when using the
 * standard {@link edu.stanford.nlp.sequences.ColumnDocumentReaderAndWriter} for
 * training or testing models, input files are expected to be one token per line
 * with the columns indicating things like the word, POS, chunk, and answer
 * class. The default for {@code ColumnDocumentReaderAndWriter} training data is
 * 3 column input, with the columns containing a word, its POS, and its gold
 * class, but this can be specified via the {@code map} property.
 * </p>
 * <p>
 * When run on a file with {@code -textFile} or {@code -textFiles}, the file is
 * assumed to be plain English text (or perhaps simple HTML/XML), and a
 * reasonable attempt is made at English tokenization by
 * {@link PlainTextDocumentReaderAndWriter}. The class used to read the text can
 * be changed with -plainTextDocumentReaderAndWriter. Extra options can be
 * supplied to the tokenizer using the -tokenizerOptions flag.
 * </p>
 * <p>
 * To read from stdin, use the flag -readStdin. The same reader/writer will be
 * used as for -textFile.
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * To test on multiple files, use the -testFiles option and a comma separated
 * list.
 * </p>
 * <p>
 ...
 * be used when training an NER classifier. There are also various feature
 * factories for Chinese word segmentation such as
 * {@link edu.stanford.nlp.wordseg.ChineseSegmenterFeatureFactory}. Features are
 * specified either by a Properties file (which is the recommended method) or by
 * flags on the command line. The flags are read into a
 * {@link SeqClassifierFlags} object, which the user need not be concerned with,
 * unless wishing to add new features.
 * </p>
 * CRFClassifier may also be used programmatically. When creating a new
 * instance, you <i>must</i> specify a Properties object. You may then call
 * train methods to train a classifier, or load a classifier. The other way to
 * get a CRFClassifier is to deserialize one via the static
 ...
 * ones in {@link AbstractSequenceClassifier}. Probabilities assigned by the CRF
 * can be interrogated using either the {@code printProbsDocument()} or
 * {@code getCliqueTrees()} methods.
 *
 * @author Jenny Finkel
...
public class CRFClassifier<IN extends CoreMap> extends AbstractSequenceClassifier<IN> {
	...
	protected List<Index<CRFLabel>> labelIndices;
	Index<String> tagIndex;
	private Pair<double[][], double[][]> entityMatrices;

	protected CliquePotentialFunction cliquePotentialFunction;
	protected HasCliquePotentialFunction cliquePotentialFunctionHelper;
	...
	protected Index<Integer> nodeFeatureIndicesMap;
	protected Index<Integer> edgeFeatureIndicesMap;
	 ...
	 * @param document
	 *            Testing documents
	 * @return A Triple, where the first element is an int[][][] representing
	 *         the data, the second element is an int[] representing the labels,
	 *         and the third element is a double[][][] representing the feature
	 *         values (optionally null)
	 */
	...
	/**
	 * Convert an ObjectBank to arrays of data features and labels. This version
	 * is used at training time.
	 *
	 * @return A Triple, where the first element is an int[][][][] representing
	 *         the data, the second element is an int[][] representing the
	 *         labels, and the third element is a double[][][][] representing
	 *         the feature values which could be optionally left as null.
	 */
	 ...
	 *
	 * @return A List of pairs, one for each document, where the first element
	 *         is an int[][][] representing the data and the second element is
	 *         an int[] representing the labels.
	 */
	...
	/**
	 * This routine builds the {@code labelIndices} which give the empirically
	 * legal label sequences (of length (order) at most {@code windowSize}) and
	 * the {@code classIndex}, which indexes known answer classes.
	 *
	 * @param ob
	 *            The training data: Read from an ObjectBank, each item in it is
	 *            a {@code List<CoreLabel>}.
	 */
	 ...
	 *
	 * @param info
	 *            The input data. Particular feature factories might look for
	 *            arbitrary keys in the IN items.
	 * @param loc
	 *            The position to build a datum at
	 * @param featureFactories
	 *            The FeatureFactories to use to extract features
	...
	/**
	 * This method is supposed to be used by CRFClassifierEvaluator only, should
	 * not have global visibility. The generic {@code classifyAndWriteAnswers}
	 * omits the second argument {@code documentDataAndLabels}.
	 */
	...
	public List<IN> classifyGibbs(List<IN> document, Triple<int[][][], int[], double[][][]> documentDataAndLabels) throws ClassNotFoundException, SecurityException, NoSuchMethodException, IllegalArgumentException, InstantiationException,
	 ...
	 * @return If verboseMode is set, a Pair of Counters recording
	 *         classification decisions, else null.
	 */
	@Override
	 ...
	 * probability distributions of the CRF. See {@code getCliqueTrees()} for
	 * more.
	 *
	 * @param filename
	...
	/**
	 * Takes the file, reads it in, and prints out the factor table at each
	 * position.
	 *
	 * @param filename
	 *            The path to the specified file
	...
	/**
	 * Takes a {@link List} of documents and prints the factor table at each
	 * point.
	 *
	 * @param documents
	 *            A {@link List} of {@link List} of INs.
	...
	/**
	 * Serialize the classifier to the given ObjectOutputStream. <br>
	...
	/**
	 * @return a matrix where each entry m[i][j] is logP(j|i) in other words,
	 *         each row vector is normalized log conditional likelihood
	 */
	...
	/**
	 * Read real-valued vector embeddings for (lowercased) word tokens. A
	 * lexicon is contained in the file flags.embeddingWords. The word vectors
	 * are then in the same order in the file flags.embeddingVectors.
	 * 
	 * @throws IOException
	 */
	 ...
	 *
	 * @param resourceName
	 *            Name of classifier resource inside the jar file.
