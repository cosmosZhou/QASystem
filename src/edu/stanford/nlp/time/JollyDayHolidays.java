...
/**
 * Wrapper around jollyday library so we can hook in holiday configurations from
 * jollyday with SUTime.
 *
 * @author Angel Chang
 */
public class JollyDayHolidays implements Env.Binder {
	...
	public void init(String prefix, Properties props) {
		...
		//		String xmlPath = props.getProperty(prefix + "xml", Utility.workingDirectory + "models/sutime/jollyday/Holidays_sutime.xml");

		String xmlPathType = props.getProperty(prefix + "pathtype", "classpath");
		//		String xmlPathType = props.getProperty(prefix + "pathtype", "file");
	...
	public static class JollyHoliday extends SUTime.Time {
		...
		public SUTime.Temporal next() {
			...
			return new SUTime.RelativeTime(new SUTime.RelativeTime(SUTime.TemporalOp.NEXT, SUTime.YEAR, SUTime.RESOLVE_TO_FUTURE), SUTime.TemporalOp.INTERSECT, this);
		}

		@Override
		public SUTime.Temporal prev() {
			// TODO: Handle holidays that are not yearly
			return new SUTime.RelativeTime(new SUTime.RelativeTime(SUTime.TemporalOp.PREV, SUTime.YEAR, SUTime.RESOLVE_TO_PAST), SUTime.TemporalOp.INTERSECT, this);
		}
