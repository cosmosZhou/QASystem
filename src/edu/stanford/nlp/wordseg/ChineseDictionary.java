...
/**
 * This class provides a main method that loads various dictionaries, and saves
 * them in a serialized version, and runtime compiles them into a word list used
 * as a feature in the segmenter.
 *
 * @author Pi-Chuan Chang
 */

public class ChineseDictionary implements Serializable {

	private static final long serialVersionUID = -2034369809862022578L;
	...
	private void addDict(String dict, boolean expandMidDot) {
		...
		for (String line : lines) {
			...
			if (expandMidDot && midDot.matcher(line).find()) {
				line = line.replaceAll(ChineseUtils.MID_DOT_REGEX_STR, "");
				if (DEBUG)
					EncodingPrintWriter.err.println("ALSO: " + line, "UTF-8");
				addOneDict(line);
			}
		}
	}
	...
	private void addOneDict(String item) {
		...
		if (length == 0) {
			...
			if (DEBUG)
				EncodingPrintWriter.err.println("DICT: " + item, "UTF-8");
			...
			if (DEBUG)
				EncodingPrintWriter.err.println("DICT: " + subItem, "UTF-8");
	...
	public static void main(String[] args) {
		...
		/*
		 * //ChineseDictionary dict = new ChineseDictionary(args[0]); for (int i
		 * = 0; i <= MAX_LEXICON_LENGTH; i++) { logger.info("Length: " +
		 * i+": "+dict.words[i].size()); } for (int i = 0; i <=
		 * MAX_LEXICON_LENGTH; i++) { logger.info("Length: " +
		 * i+": "+dict.words[i].size()); if (dict.words[i].size() < 1000) { for
		 * (String word : dict.words[i]) { EncodingPrintWriter.err.println(word,
		 * "UTF-8"); } } } for (int i = 1; i < args.length; i++) {
		 * logger.info(args[i] + " " +
		 * Boolean.valueOf(dict.contains(args[i])).toString()); }
		 */
	}
