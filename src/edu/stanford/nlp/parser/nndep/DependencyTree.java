...
/**
 * Represents a partial or complete dependency parse of a sentence, and provides
 * convenience methods for analyzing the parse.
 *
 * @author Danqi Chen
 */
public class DependencyTree {
	 ...
	 * @param l
	 *            Label of the dependency relation
	 */
	...
	public int getParent(int k) {
		return head.get(k + 1) - 1;
	}

	public String getDepLabel(int k) {
		return label.get(k + 1);
	}

	/**
	 * Get the index of the node which is the root of the parse (i.e., that node
	 * which has the ROOT node as its head).
	 */
	public int getRoot() {
		for (int k = 1; k <= n; ++k)
			if (getHead(k) == 0)
	...
	public String toString() {
		...
		for (int i = 0; i < head.length; i++) {
			head[i] = Integer.toString(this.head.get(i + 1) - 1);
			label[i] = this.label.get(i + 1);
		}
		String[] res = Utility.convertWithAlignment(head, label);
