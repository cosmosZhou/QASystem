...
public class RNNCostAndGradient extends AbstractCachingDiffFunction {

	private final RNNModel model;
	private final List<Tree> trainingBatch;

	public RNNCostAndGradient(RNNModel model, List<Tree> trainingBatch) {
		this.model = model;
	...
	/**
	 * Returns the index with the highest value in the <code>predictions</code>
	 * matrix. Indexed from 0.
	 */
	...
	private static class ModelDerivatives {
		...
		public ModelDerivatives(RNNModel model) {
			binaryTD = initDerivatives(model.binaryTransform);
			binaryTensorTD = (model.op.useTensors) ? initTensorDerivatives(model.binaryTensors) : Generics.newTreeMap();
			binaryCD = (!model.op.combineClassification) ? initDerivatives(model.binaryClassification) : Generics.newTreeMap();
		 ...
		 */
		public static void addTensors(Map<String, SimpleTensor> first, Map<String, SimpleTensor> second) {
			for (Map.Entry<String, SimpleTensor> entry : first.entrySet()) {
				if (second.containsKey(entry.getKey())) {
					first.put(entry.getKey(), entry.getValue().plus(second.get(entry.getKey())));
				}
			}

			for (Map.Entry<String, SimpleTensor> entry : second.entrySet()) {
		...
		/**
		 * Init a TwoDimensionalMap with 0 matrices for all the matrices in the
		 * original map.
		 */
		...
		/**
		 * Init a TwoDimensionalMap with 0 tensors for all the tensors in the
		 * original map.
		 */
		private static Map<String, SimpleTensor> initTensorDerivatives(Map<String, SimpleTensor> map) {
			Map<String, SimpleTensor> derivatives = Generics.newTreeMap();

			for (Map.Entry<String, SimpleTensor> entry : map.entrySet()) {
				int numRows = entry.getValue().numRows();
				int numCols = entry.getValue().numCols();
				int numSlices = entry.getValue().numSlices();

				derivatives.put(entry.getKey(), new SimpleTensor(numRows, numCols, numSlices));
			}
	...
	private ModelDerivatives scoreDerivativesForPolarityClassification(List<Tree> trainingBatch) {
		// "final" makes this as fast as having separate maps declared in this function
		final ModelDerivatives derivatives = new ModelDerivatives(model);
		...
		for (Tree tree : forwardPropTrees) {
			backpropDerivativesAndErrorForPolarityClassification(tree, derivatives.binaryTD, derivatives.binaryCD, derivatives.binaryTensorTD, derivatives.unaryCD, derivatives.wordVectorD);
			derivatives.error += sumError(tree);
		}
	...
	public void calculate(double[] theta) {
		...
		derivative = NeuralUtils.paramsToVector(theta.length, derivatives.binaryTD.values().iterator(), derivatives.binaryCD.values().iterator(), SimpleTensor.iteratorSimpleMatrix(derivatives.binaryTensorTD.values().iterator()), derivatives.unaryCD.values().iterator(), derivatives.wordVectorD.values().iterator());
	}

	public void calculateForPolarityClassification(double[] theta) {
		model.vectorToParams(theta);

		final ModelDerivatives derivatives;
		if (model.op.trainOptions.nThreads == 1) {
			derivatives = scoreDerivativesForPolarityClassification(trainingBatch);
		} else {
			// TODO: because some addition operations happen in different
			// orders now, this results in slightly different values, which
			// over time add up to significantly different models even when
	...
	static double scaleAndRegularize(Map<String, SimpleMatrix> derivatives, Map<String, SimpleMatrix> currentMatrices, double scale, double regCost, boolean dropBiasColumn) {
	...
	static double scaleAndRegularize(Map<String, SimpleMatrix> derivatives, Map<String, SimpleMatrix> currentMatrices, double scale, double regCost, boolean activeMatricesOnly, boolean dropBiasColumn) {
		double cost = 0.0; // the regularization cost
		for (Map.Entry<String, SimpleMatrix> entry : currentMatrices.entrySet()) {
			SimpleMatrix D = derivatives.get(entry.getKey());
	...
	static double scaleAndRegularizeTensor(Map<String, SimpleTensor> derivatives, Map<String, SimpleTensor> currentMatrices, double scale, double regCost) {
		double cost = 0.0; // the regularization cost
		for (Map.Entry<String, SimpleTensor> entry : currentMatrices.entrySet()) {
			SimpleTensor D = derivatives.get(entry.getKey());
	...
	private void backpropDerivativesAndError(Tree tree, Map<String, SimpleMatrix> binaryTD, Map<String, SimpleMatrix> binaryCD, Map<String, SimpleTensor> binaryTensorTD, Map<String, SimpleMatrix> unaryCD, Map<String, SimpleMatrix> wordVectorD) {
	...
	private void backpropDerivativesAndError(Tree tree, Map<String, SimpleMatrix> binaryTD, Map<String, SimpleMatrix> binaryCD, Map<String, SimpleTensor> binaryTensorTD, Map<String, SimpleMatrix> unaryCD, Map<String, SimpleMatrix> wordVectorD,
		...
		if (tree.isPreTerminal()) { // below us is a word vector
			...
			if (word == null) {
				throw new RuntimeException(tree.children()[0].label().value() + " not found!");
			}
			...
			String binarCategory = model.basicCategory(tree.label().value());

			if (model.op.combineClassification) {
				unaryCD.put("", unaryCD.get("").plus(localCD));
			} else {
				binaryCD.put(binarCategory, binaryCD.get(binarCategory).plus(localCD));
			}

			SimpleMatrix currentVectorDerivative = NeuralUtils.elementwiseApplyTanhDerivative(currentVector);
			SimpleMatrix deltaFromClass = model.getBinaryClassification(binarCategory).transpose().mult(deltaClass);
			deltaFromClass = deltaFromClass.extractMatrix(0, model.op.numHid, 0, 1).elementMult(currentVectorDerivative);
			SimpleMatrix deltaFull = deltaFromClass.plus(deltaUp);
			...
			binaryTD.put(binarCategory, binaryTD.get(binarCategory).plus(W_df));
			SimpleMatrix deltaDown;
			if (model.op.useTensors) {
				SimpleTensor Wt_df = getTensorGradient(deltaFull, leftVector, rightVector);
				binaryTensorTD.put(binarCategory, binaryTensorTD.get(binarCategory).plus(Wt_df));
				deltaDown = computeTensorDeltaDown(deltaFull, leftVector, rightVector, model.getBinaryTransform(binarCategory), model.getBinaryTensor(binarCategory));
			} else {
				deltaDown = model.getBinaryTransform(binarCategory).transpose().mult(deltaFull);
			}
	...
	private void backpropDerivativesAndErrorForPolarityClassification(Tree tree, Map<String, SimpleMatrix> binaryTD, Map<String, SimpleMatrix> binaryCD, Map<String, SimpleTensor> binaryTensorTD, Map<String, SimpleMatrix> unaryCD,
			Map<String, SimpleMatrix> wordVectorD) {
		SimpleMatrix delta = new SimpleMatrix(model.op.numHid, 1);
		backpropDerivativesAndErrorForPolarityClassification(tree, binaryTD, binaryCD, binaryTensorTD, unaryCD, wordVectorD, delta);
	}

	private void backpropDerivativesAndErrorForPolarityClassification(Tree tree, Map<String, SimpleMatrix> binaryTD, Map<String, SimpleMatrix> binaryCD, Map<String, SimpleTensor> binaryTensorTD, Map<String, SimpleMatrix> unaryCD,
			Map<String, SimpleMatrix> wordVectorD, SimpleMatrix deltaUp) {
		if (tree.isLeaf()) {
			return;
		}
		...
		// Build a vector that looks like 0,0,1,0,0 with an indicator for the correct class
		SimpleMatrix goldLabel = new SimpleMatrix(model.numClasses, 1);
		double goldScore = RNNCoreAnnotations.getGoldScore(tree);

		goldLabel.set(1, goldScore);
		goldLabel.set(0, 1 - goldScore);

		double nodeWeight = 1.0;
		//model.op.trainOptions.getClassWeight(goldClass);
		...
		// calculations, but this would be the easiest way to handle the
		// unlabeled class
		SimpleMatrix deltaClass = goldScore >= 0 ? predictions.minus(goldLabel).scale(nodeWeight) : new SimpleMatrix(predictions.numRows(), predictions.numCols());
		SimpleMatrix localCD = deltaClass.mult(NeuralUtils.concatenateWithBias(currentVector).transpose());
		...
		if (tree.isPreTerminal()) { // below us is a word vector
			...
			SimpleMatrix leftDeltaDown = deltaDown.extractMatrix(0, deltaFull.numRows(), 0, 1);
			SimpleMatrix rightDeltaDown = deltaDown.extractMatrix(deltaFull.numRows(), deltaFull.numRows() * 2, 0, 1);
			backpropDerivativesAndErrorForPolarityClassification(tree.children()[0], binaryTD, binaryCD, binaryTensorTD, unaryCD, wordVectorD, leftDerivative.elementMult(leftDeltaDown));
			backpropDerivativesAndErrorForPolarityClassification(tree.children()[1], binaryTD, binaryCD, binaryTensorTD, unaryCD, wordVectorD, rightDerivative.elementMult(rightDeltaDown));
		}
	}
	...
	/**
	 * This is the method to call for assigning labels and node vectors to the
	 * Tree. After calling this, each of the non-leaf nodes will have the node
	 * vector and the predictions of their classes assigned to that subtree's
	 * node. The annotations filled in are the RNNCoreAnnotations.NodeVector,
	 * Predictions, and PredictedClass. In general, PredictedClass will be the
	 * most useful annotation except when training.
	 */
	public void forwardPropagateTree(Tree tree) {
		...
		if (tree.isLeaf()) {
			...
			if (classification == null) {
				classification = model.getUnaryClassification(tree.label().value());
				throw new RuntimeException(tree.label().value() + ": unaryCategory not found");
			}
			...
			String binaryCategory = tree.label().value();
			//			String leftCategory = tree.children()[0].label().value();
			//			String rightCategory = tree.children()[1].label().value();
			SimpleMatrix W = model.getBinaryTransform(binaryCategory);
			classification = model.getBinaryClassification(binaryCategory);

			if (classification == null) {
				throw new RuntimeException(binaryCategory + ": binaryCategory not found");
			}
			...
			SimpleMatrix childrenVector = NeuralUtils.concatenateWithBias(leftVector, rightVector);
			if (model.op.useTensors) {
				SimpleTensor tensor = model.getBinaryTensor(binaryCategory);
	...
	public double[] derivativeAtForPolarityClassification(double[] x) {
		ensureForPolarityClassification(x);
		return derivative;
	}

	public void ensureForPolarityClassification(double[] x) {
		...
		copy(lastX, x);
		fEvaluations += 1;
		calculateForPolarityClassification(x);
	}
