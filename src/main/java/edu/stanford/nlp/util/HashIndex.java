...
/**
 * Implements an Index that supports constant-time lookup in both directions
 * (via {@code get(int)} and {@code indexOf(E)}. The {@code indexOf(E)} method
 * compares objects by {@code equals()}, as other Collections.
 * <p/>
 * The typical usage would be:
 * <p>
 * {@code Index<String> index = new Index<String>(collection);}
 * <p>
 * followed by
 * <p>
 ...
 * <p>
 * An Index can be locked or unlocked: a locked index cannot have new items
 * added to it.
 *
 ...
 * @since 1.0
 * @author <a href="mailto:yeh1@stanford.edu">Eric Yeh</a> (added write to/load
 *         from buffer)
 */
...
public class HashIndex<E> extends AbstractCollection<E> implements Index<E>, RandomAccess {
	...
	/**
	 * Looks up the objects corresponding to an array of indices, and returns
	 * them in a {@link Collection}. This collection is not a copy, but accesses
	 * the data structures of the Index.
	 *
	 * @param indices
	 *            An array of indices
	 * @return a {@link Collection} of the objects corresponding to the indices
	 *         argument.
	 */
	@Override
	...
	/**
	 * Returns a complete {@link List} of indexed objects, in the order of their
	 * indices. <b>DANGER!</b> The current implementation returns the actual
	 * index list, not a defensive copy. Messing with this List can seriously
	 * screw up the state of the Index. (perhaps this method needs to be
	 * eliminated? I don't think it's ever used in ways that we couldn't use the
	 * Index itself for directly. --Roger, 12/29/04)
	 *
	...
	/**
	 * Locks the Index. A locked index cannot have new elements added to it
	 * (calls to {@link #add} will leave the Index unchanged and return
	 * {@code false}).
	 */
	@Override
	public void lock() {
	...
	/**
	 * Unlocks the Index. A locked index cannot have new elements added to it
	 * (calls to {@link #add} will leave the Index unchanged and return
	 * {@code false}).
	 */
	@Override
	...
	/**
	 * Add the given item to the index, but without taking any locks. Use this
	 * method with care! But, this offers a noticable performance improvement if
	 * it is safe to use.
	 *
	...
	/**
	 * Takes an Object and returns the integer index of the Object, perhaps
	 * adding it to the index first. Returns -1 if the Object is not in the
	 * Index.
	 * <p>
	 ...
	 * @return The index of the Object argument. Returns -1 if the object is not
	 *         in the index.
	 */
	@Override
	@Deprecated
	...
	/**
	 * Adds every member of Collection to the Index. Does nothing for members
	 * already in the Index.
	 *
	 * @return true if some item was added to the index and false if no item was
	 *         already in the index or if the index is locked
	 */
	@Override
	...
	/**
	 * Adds an object to the Index. If it was already in the Index, then nothing
	 * is done. If it is not in the Index, then it is added iff the Index hasn't
	 * been locked.
	 *
	 * @return true if the item was added to the index and false if the item was
	 *         already in the index or if the index is locked
	 */
	@Override
	...
	/**
	 * Create a new <code>HashIndex</code>, backed by the given collection
	 * types.
	 * 
	 * @param objLookupFactory
	 *            The constructor for the object lookup -- traditionally an
	 *            {@link ArrayList}.
	 * @param indexLookupFactory
	...
	/**
	 * This assumes each line is of the form (number=value) and it adds each
	 * value in order of the lines in the file. Warning: This ignores the value
	 * of number, and just indexes each value it encounters in turn!
	 *
	 * @param file
	 *            Which file to load
	...
	/**
	 * This saves the contents of this index into string form, as part of a
	 * larger text-serialization. This is not intended to act as a standalone
	 * routine, instead being called from the text-serialization routine for a
	 * component that makes use of an Index, so everything can be stored in one
	 * file. This is similar to {@code saveToFileName}.
	 * 
	 * @param bw
	 *            Writer to save to.
	 * @throws IOException
	 *             Exception thrown if cannot save.
	 */
	@Override
	...
	/**
	 * This is the analogue of {@code loadFromFilename}, and is intended to be
	 * included in a routine that unpacks a text-serialized form of an object
	 * that incorporates an Index. NOTE: presumes that the next readLine() will
	 * read in the first line of the portion of the text file representing the
	 * saved Index. Currently reads until it encounters a blank line, consuming
	 * that line and returning the Index. TODO: figure out how best to
	 * terminate: currently a blank line is considered to be a terminator.
	 * 
	 * @param br
	 ...
	 *
	 * @param n
	 *            Show the first <i>n</i> items in the Index
	...
	/**
	 * Returns an unmodifiable view of the Index. It is just a locked index that
	 * cannot be unlocked, so if you try to add something, nothing will happen
	 * (it won't throw an exception). Trying to unlock it will throw an
	 * UnsupportedOperationException. If the underlying Index is modified, the
	...
	/**
	 * This assumes each line is one value and creates index by adding values in
	 * the order of the lines in the file
	 * 
	 * @param file
	 *            Which file to load
	 * @return An index built out of the lines in the file
	...
	public void set(int index, E obj) {
		...
		if (old == null || index >= objects.size()) {
			throw new NullPointerException("old == null");
		}
		objects.set(index, obj);
		indexes.remove(old);
		indexes.put(obj, index);
		if (objects.size() != indexes.size()){
			throw new RuntimeException("objects.size() != indexes.size()");
		}
	}
}
