...
/**
 * A class capable of computing the best sequence given a SequenceModel. Uses
...
public class ExactBestSequenceFinder implements BestSequenceFinder {
	...
	private static Pair<int[], Double> bestSequence(SequenceModel ts, double[][] linearConstraints) {
		...
		if (DEBUG) {
			...
			System.out.println("Doing bestSequence length " + length + "; leftWin " + leftWindow + "; rightWin " + rightWindow + "; padLength " + padLength);
		}
		for (int pos = 0; pos < padLength; pos++) {
			...
			if (DEBUG) {
				System.out.println("There are " + tagNum[pos] + " values at position " + pos + ": " + Arrays.toString(tags[pos]));
			}
		}
		...
		if (DEBUG) {
			System.out.println("tags = " + Utility.toString(tags));
			System.out.println("tagNum = " + Utility.toString(tagNum));
			System.out.println("productSizes = " + Utility.toString(productSizes));
		}
		...
		for (int pos = leftWindow; pos < leftWindow + length; pos++) {
			...
			if (DEBUG) {
				System.out.println("scoring word " + pos + " / " + (leftWindow + length) + ", productSizes =  " + productSizes[pos] + ", tagNum = " + tagNum[pos] + "...");
			}
			windowScore[pos] = new double[productSizes[pos]];
			Arrays.fill(tempTags, tags[0][0]);
			if (DEBUG) {
				System.out.println("windowScore[" + pos + "] has size (productSizes[pos]) " + windowScore[pos].length);
			}
			...
			for (int product = 0; product < productSizes[pos]; product++) {
				...
				if (tempTags[pos] == tags[pos][0]) {
					...
					if (DEBUG) {	
						System.out.println("ts = " + ts);
						System.out.println("tempTags = " + Utility.toString(tempTags));
						System.out.println("Matched at array index [product] " + product + "; tempTags[pos] == tags[pos][0] == " + tempTags[pos]);
						System.out.println("For pos " + pos + " scores.length is " + scores.length + "; tagNum[pos] = " + tagNum[pos] + "; windowScore[pos].length = " + windowScore[pos].length);
						System.out.println("scores: " + Arrays.toString(scores));
						log.info("ts = " + ts);
						log.info("tempTags = " + Utility.toString(tempTags));
					...
					for (int t = 0; t < tagNum[pos]; t++) {
						if (DEBUG) {
							System.out.println("Setting value of windowScore[" + pos + "][" + product + "+" + t + "*" + shift + "] = " + scores[t]);
						}
					...
					if (linearConstraints != null) {
						if (DEBUG) {
							if (linearConstraints[pos][product % tagNum[pos]] != 0) {
								System.out.println("Applying linear constraints=" + linearConstraints[pos][product % tagNum[pos]] + " to preScore=" + windowScore[pos][product] + " at pos=" + pos + " for tag=" + (product % tagNum[pos]));
							}
						}
						...
						if (linearConstraints != null) {
							if (DEBUG) {
								if (pos == 2 && linearConstraints[pos][product % tagNum[pos]] != 0) {
									System.out.println("Applying linear constraints=" + linearConstraints[pos][product % tagNum[pos]] + " to preScore=" + predScore + " at pos=" + pos + " for tag=" + (product % tagNum[pos]));
									System.out.println("predScore:" + predScore + " = score[" + (pos - 1) + "][" + predProduct + "]:" + score[pos - 1][predProduct] + " + windowScore[" + pos + "][" + product + "]:" + windowScore[pos][product]);
								}
							}
