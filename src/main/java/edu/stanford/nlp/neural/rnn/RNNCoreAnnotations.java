...
public class RNNCoreAnnotations {
	...
	/**
	 * Used to denote the vector (distributed representation) at a particular
	 * node. This stores a real vector that represents the semantics of a word
	 * or phrase.
	 */
	...
	/**
	 * Used to denote a vector of predictions at a particular node. This is a
	 * vector of real values, typically the output of a softmax classification
	 * layer, which gives the probabilities of each output value.
	 */
	...
	/**
	 * Get the argmax of the class predicteions. The predicted classes can be an
	 * arbitrary set of non-negative integer classes, but in our current
	 * sentiment models, the values used are on a 5-point scale of 0 = very
	...
	/**
	 * Return as an int the predicted class. If it is not defined for a node, it
	 * will return -1
	 *
	...
	public static int getGoldClass(Tree tree) {
		...
		if (coreLabel.containsKey(GoldClass.class))
			return coreLabel.get(GoldClass.class);
		return -1;
	}
	 ...
	 * The index of the correct class.
	 */
	public static class GoldScore implements CoreAnnotation<Double> {
		@Override
	...
	public static double getGoldScore(Tree tree) {
		Label label = tree.label();
		if (!(label instanceof CoreLabel)) {
			throw new IllegalArgumentException("CoreLabels required to get the attached gold class");
		}
		...
		CoreLabel coreLabel = (CoreLabel) label;
		if (coreLabel.containsKey(GoldScore.class))
			return coreLabel.get(GoldScore.class);
		return -1;
	}

	public static void setGoldScore(Tree tree, double goldClass) {
		Label label = tree.label();
		if (!(label instanceof CoreLabel)) {
			throw new IllegalArgumentException("CoreLabels required to set the attached gold class");
		}
		((CoreLabel) label).set(GoldScore.class, goldClass);
	}
