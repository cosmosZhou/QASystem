...
/**
 * This class defines a transition-based dependency parser which makes use of a
 * classifier powered by a neural network. The neural network accepts
 * distributed representation inputs: dense, continuous representations of
 * words, their part of speech tags, and the labels which connect words in a
 * partial dependency parse.
 *
 * <p>
 * This is an implementation of the method described in
 *
 * <blockquote> Danqi Chen and Christopher Manning. A Fast and Accurate
 * Dependency Parser Using Neural Networks. In EMNLP 2014. </blockquote>
 *
 * <p>
 * New models can be trained from the command line; see {@link #main} for
 * details on training options. This parser will also output CoNLL-X format
 * predictions; again see {@link #main} for available options.
 *
 * <p>
 * This parser can also be used programmatically. The easiest way to prepare the
 * parser with a pre-trained model is to call
 * {@link #loadFromModelFile(String)}. Then call
 * {@link #predict(edu.stanford.nlp.util.CoreMap)} on the returned parser
 * instance in order to get new parses.
 *
...
public class DependencyParser {
	 ...
	 */
	protected List<String> knownWords, knownPos, knownLabels;
	...
	/**
	 * Given a particular parser configuration, this classifier will predict the
	 * best transition to make next.
	 *
			...
			if (trees.get(i).isProjective()) {
				...
				while (!system.isTerminal(c)) {
					...
					for (int j = 0; j < feature.size(); ++j) {
						if (feature.get(j) >= mE) {
							throw new RuntimeException("feature.get(" + j + ") = " + feature.get(j) + " >= " + mE);
						}
	 ...
	 */
	protected void setupClassifierForTraining(List<CoreMap> trainSents, List<DependencyTree> trainTrees, String embedFile, String preModel) {
	 ...
	 * purposes.
	 */
	public DependencyTree predictInner(CoreMap sentence) {
		int numTrans = system.numTransitions();
	...
	public double testCoNLL(String testFile, String outFile) {
		...
		System.err.printf("%s parsed %d words in %d sentences in %.1fs at %.1f w/s, %.1f sent/s.%n", StringUtils.getShortClassName(this), numWords, numSentences, millis / 1000.0, wordspersec, sentspersec);
	 ...
	 * <td>edu/stanford/nlp/models/pos-tagger/english-left3words/english-left3words-distsim.tagger</td>
