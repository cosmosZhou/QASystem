 ...
 * be sure to call {@link #finalizeTraining()} in order to allow the classifier
 * to clean up resources used during training.
 *
 * @author Danqi Chen
 * @author Jon Gauthier
 */
public class Classifier {
	...
	/**
	 * Evaluates the training cost of a particular subset of training examples
	 * given the current learned weights.
	 *
	 * This function will be evaluated in parallel on different data in separate
	 * threads, and accesses the classifier's weights stored in the outer class
	 * instance.
	 *
	 * Each nested class instance accumulates its own weight gradients; these
	 * gradients will be merged on a main thread after all cost function runs
	 * complete.
	 *
	...
	private class CostFunction implements ThreadsafeProcessor<Pair<Collection<Example>, FeedforwardParams>, Cost> {
		...
		public Cost process(Pair<Collection<Example>, FeedforwardParams> input) {
			...
			for (Example ex : examples) {
				...
				int[] ls = IntStream.range(0, config.hiddenSize).filter(n -> random.nextDouble() > params.getDropOutProb()).toArray();
	...
	public void preCompute(Set<Integer> toPreCompute) {
		...
		for (int x : toPreCompute) {
			...
			for (int j = 0; j < config.hiddenSize; ++j)
				for (int k = 0; k < config.embeddingSize; ++k) {
					if (tok >= E.length) {
						log.info("java.lang.ArrayIndexOutOfBoundsException: " + tok);
					}
