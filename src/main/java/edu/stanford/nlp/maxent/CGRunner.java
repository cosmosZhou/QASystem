 ...
 * Quasi-Newton on a LambdaSolve object to find optimal parameters, including
 * imposing a Gaussian prior on those parameters.
 *
...
public class CGRunner {
	...
	/**
	 * Set up a LambdaSolve problem for solution by a Minimizer. Uses a Gaussian
	 * prior with a sigma<sup>2</sup> of 0.5.
	 *
	 * @param prob
	 *            The problem to solve
	 * @param filename
	 *            Used (with extension) to save intermediate results.
	 */
	...
	/**
	 * Set up a LambdaSolve problem for solution by a Minimizer, specifying a
	 * value for sigma<sup>2</sup>.
	 *
	 * @param prob
	 *            The problem to solve
	 * @param filename
	 *            Used (with extension) to save intermediate results.
	 * @param priorSigmaS
	 *            The prior sigma<sup>2</sup>: this doubled will be used to
	 *            divide the lambda<sup>2</sup> values as the prior penalty in
	 *            the likelihood. A value of 0.0 or Double.POSITIVE_INFINITY
	 ...
	 *            Used (with extension) to save intermediate results.
	 * @param tol
	 *            Tolerance of errors (passed to CG)
	 * @param priorSigmaS
	 *            The prior sigma<sup>2</sup>: this doubled will be used to
	 *            divide the lambda<sup>2</sup> values as the prior penalty. A
	 *            value of 0.0 or Double.POSITIVE_INFINITY indicates to not use
	 *            regularization.
	 */
	 ...
	 *            Tolerance of errors (passed to CG)
	 * @param sigmaSquareds
	 *            The prior sigma<sup>2</sup> for each feature: this doubled
	 *            will be used to divide the lambda<sup>2</sup> values as the
	 *            prior penalty. This array must have size the number of
	 *            features. If it is null, no regularization will be performed.
	 */
	...
	/**
	 * Solves the problem using a quasi-newton method (L-BFGS). The solution is
	 * stored in the {@code lambda} array of {@code prob}.
	 */
	public void solveQN(double[] lambda) {
		...
		if (lambda.length != df.domainDimension()) {
			throw new RuntimeException("lambda.length != df.domainDimension()");
		}

		prob.lambda = cgm.minimize(df, tol, lambda);
	...
	public void solveOWLQN2(double weight, double lambda[]) {
		LikelihoodFunction df = new LikelihoodFunction(prob, tol, useGaussianPrior, priorSigmaS, sigmaSquareds);
		MonitorFunction monitor = new MonitorFunction(prob, df, filename);
		Minimizer<DiffFunction> cgm = new QNMinimizer(monitor, 10);
		((QNMinimizer) cgm).useOWLQN(true, weight);
	...
	/**
	 * Solves the problem using conjugate gradient (CG). The solution is stored
	 * in the {@code lambda} array of {@code prob}.
	 */
	public void solveCG() {
		LikelihoodFunction df = new LikelihoodFunction(prob, tol, useGaussianPrior, priorSigmaS, sigmaSquareds);
		MonitorFunction monitor = new MonitorFunction(prob, df, filename);
	...
	public void solveCG(double lambda[]) {
		LikelihoodFunction df = new LikelihoodFunction(prob, tol, useGaussianPrior, priorSigmaS, sigmaSquareds);
		MonitorFunction monitor = new MonitorFunction(prob, df, filename);
		Minimizer<DiffFunction> cgm = new CGMinimizer(monitor);
	 ...
	 * {@code lambda} array of {@code prob}. Note that the likelihood function
	 * will be a penalized L2 likelihood function unless you have turned this
	 * off via setting the priorSigmaS to 0.0.
	 *
	 * @param weight
	 *            Controls the sparseness/regularization of the L1 solution. The
	 *            bigger the number the sparser the solution. Weights between
