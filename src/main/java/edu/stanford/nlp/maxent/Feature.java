...
/**
 * This class is used as a base class for TaggerFeature for the tagging problem
 * and for BinaryFeature for the general problem with binary features.
 *
...
public class Feature {
	 ...
	 * case it is sparse. The pairs (x,y) are coded as x*ySize+y. The values are
	 * kept in valuesI. For example, if a feature has only two non-zero values,
	 * e.g f(1,2)=3 and f(6,3)=0.74, then indexedValues will have values
	...
	/**
	 * This is if we are given an array of double with a value for each training
	 * sample in the order of their occurrence.
	 */
	...
	/**
	 * @param indexes
	 *            The pairs (x,y) for which the feature is non-zero. They are
	 *            coded as x*ySize+y
	 * @param vals
	...
	/**
	 * Prints out the points where the feature is non-zero and the values at
	 * these points.
	 */
	public void print() {
		print(System.out);
	...
	/**
	 * Used to sequentially set the values of a feature -- index is the pace in
	 * the arrays ; key goes into indexedValues, and value goes into valuesI.
	 */
	...
	/**
	 * This is rarely used because it is slower and requires initHashVals() to
	 * be called beforehand to initialize the hashValues.
	 */
	...
	/**
	 * Creates a HashMap with keys indices from pairs (x,y) and values the value
	 * of the function at the pair; required for use of getVal(x,y)
	 */
	...
	public String toString() {
		String str = "";
		str += "indexedValues = ";
		str += Utility.toString(indexedValues);

		str += "\t";
		str += "valuesI = ";
		str += Utility.toString(valuesI);

		str += "\t";
		str += "hashValues = ";

		str += hashValues == null ? null : Utility.toString(hashValues);
