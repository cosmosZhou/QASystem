...
public class Experiments {
	...
	/**
	 * vArray has dimensions [numTrainingDatums][2] and holds the x and y (word
	 * and tag index) for each training sample. Its length is the number of data
	 * points.
	 */
	...
	/**
	 * Maximum ySize. todo [CDM May 2007]: What is this and what does it
	 * control? Why isn't it set dynamically? Is it the number of different y
	 * values that one x value can have? If so, although it was set to 5, it
	 * should be 7 for the WSJ PTB. But that doesn't solve the problem for the
	 * data set after that.... See the commented out bits where it should
	 * exception if it overflows. Should just be able to make it dynamic
	 */
	...
	/**
	 * If this constructor is used, the maximum possible class overall is found
	 * and all classes are assumed possible for all instances.
	 */
	...
	/**
	 * The number of possible classes for each instance is contained in the
	 * array maxYs then the possible classes for x are from 0 to maxYs[x]-1.
	 */
	 ...
	 * {@literal <data><xSize>xSize</xSize><ySize>ySize</ySize>} x1 y1 x2 y2 ..
	 * {@literal </data>} ..
	 */
	...
	public final void ptilde() {
		...
		System.out.println("in all, there " + px.length + " different unique features extracted in the history.");
		System.out.println("in all, there " + py.length + " different tags in the history");
...
//			if (px[xC] > 1)
//				System.out.println("feature indexed " + xC + " is used for than twice.");
			py[yC]++;
			for (int j = 0; j < dim; j++) {
...
//			System.out.println(" Exception more than  " + dim);
		...
		for (int y = 0; y < ySize; y++) {
			...
			//ptildeY(y) - sum = 0 should hold in theory!
	...
	public double ptildeY(int y) {
		if (y > ySize - 1) {
			return 0.0;
		}//y <= ySize - 1, ie y < ySize
		return py[y] / (double) size();
	}

	//return the frequency that x and y co-occurr.
