...
public class TTags implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 5866676093213522716L;
	...
	/**
	 * When making a decision based on the training data as to whether a tag is
	 * closed, this is the threshold for how many tokens can be in a closed
	 * class - purposely conservative. TODO: make this an option you can set;
	 * need to pass in TaggerConfig object and then can say =
	 * config.getClosedTagThreshold());
	 */
	...
	/*
	 * public TTags(TaggerConfig config) { String[] closedArray =
	 * config.getClosedClassTags(); String[] openArray =
	 * config.getOpenClassTags(); if(closedArray.length > 0) { closed =
	 * Generics.newHashSet(Arrays.asList(closedArray)); } else
	 * if(openArray.length > 0) { openTags =
	 * Generics.newHashSet(Arrays.asList(openArray)); } else { learnClosedTags =
	 * config.getLearnClosedClassTags(); closedTagThreshold =
	 * config.getClosedTagThreshold(); } }
	 */
	...
	/**
	 * Return the Set of tags used by this tagger (available after training the
	 * tagger).
	 *
	 ...
	 * For instance, for English with the Penn POS tag, a word with the VB tag
	 * would also be expected to have the VBP tag.
	 * <p>
	 ...
	 * intuitively it's just: For English (only), if the VBD tag is present but
	 * not VBN, add it, and vice versa; if the VB tag is present but not VBP,
	 * add it, and vice versa.
	 *
	 * @param tags
	...
	public void swap(String oldTag, String newTag) {
		HashIndex<String> index = (HashIndex<String>) this.index;
		if (index.contains(oldTag)) {
			index.set(index.indexOf(oldTag), newTag);

			if (closed.contains(oldTag)) {
				closed.remove(oldTag);
				closed.add(newTag);
			} else  {
				if (!openTags.contains(oldTag)){
					throw new RuntimeException("!openTags.contains(oldTag)");
				}
				openTags.remove(oldTag);
				openTags.add(newTag);
			}
		}
	}
