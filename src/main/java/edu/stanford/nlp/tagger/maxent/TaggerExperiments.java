 ...
 * for example the frequency of each x or y in the training data.
 *
...
public class TaggerExperiments extends Experiments {
	...
	/**
	 * This method gets feature statistics from a training file found in the
	 * TaggerConfig. It is the start of the training process.
	 */
	public TaggerExperiments(TaggerConfig config, MaxentTagger maxentTagger) throws IOException {
		this(maxentTagger);
		...
		log.info("vArray.length = " + vArray.length);
		log.info("totalWords = " + c.totalWords);
		log.info("numElements = " + c.numElements);
		log.info("totalSentences = " + c.totalSentences);
		initTemplatesNew();
	...
	public TaggerExperiments(TaggerConfig config, MaxentTagger maxentTagger, boolean bSequential) throws IOException {
		this(maxentTagger);
		...
		if (bSequential) {
			getFeaturesNewSequentially();
		} else {
			maxentTagger.fAssociations = Generics.newArrayList();
			getFeaturesNew();
		}
	}
	...
	/**
	 * This method uses and deletes a file tempXXXXXX.x in the current
	 * directory!
	 */
...
//			log.info("sTemplates = " + Utility.toString(sTemplates, "\n"));
	...
	private void getFeaturesNewSequentially() throws IOException {
		// todo: Change to rethrow a RuntimeIOException.
		// todo: can fnumArr overflow?

		log.info("TaggerExperiments.getFeaturesNew: initializing fnumArr.");
		fnumArr = new byte[xSize][ySize]; // what is the maximum number of active features
		File hFile = File.createTempFile("temp", ".x", new File(Utility.workingDirectory));
		RandomAccessFile hF = new RandomAccessFile(hFile, "rw");
		log.info("  length of sTemplates keys: " + sTemplates.size());
//		log.info("sTemplates = " + Utility.toString(sTemplates, "\n"));
		log.info("getFeaturesNew adding features ...");
		...
		double[] lambda = maxentTagger.prob.lambda.clone();
		HashSet<FeatureKey> activeFeatureSet = new HashSet<FeatureKey>();
		if (feats.size() > 0)
			throw new RuntimeException("feats.size() > 0");
		...
		for (FeatureKey fK : sTemplates) {
			...
			if (positions == null) {
				...
				tF = new TaggerFeature(current, current + numElements - 1, fK, maxentTagger.getTagIndex(fK.tag), this);
				tFeature.addPositions(current, current + numElements - 1, fK);
				current = current + numElements;
				//						log.info("  added feature with key " + fK + " has support " + numElements);
				...
				// this is the second time to write these values
				tF = new TaggerFeature(positions[0], positions[1], fK, maxentTagger.getTagIndex(fK.tag), this);

				//						log.info("  added feature with key " + fK + " has support " + xValues.length);
			}
			feats.add(tF);
			activeFeatureSet.add(fK);
			...
			int tagIndex = maxentTagger.tags.getIndex(fK.tag);
			if (tagIndex >= fTagAssociations.length || tagIndex < 0) {
				log.info("tagIndex >= fTagAssociations.length || tagIndex < 0");
				log.info("fK.tag = " + fK.tag);
				throw new RuntimeException("Array Index Out Of Bounds Exception");
			}
			int pointer = fTagAssociations[tagIndex];
			if (pointer != numFeats) {
				if (lambda.length <= numFeats) {
					lambda = Arrays.copyOf(lambda, numFeats * 2);
				}

				if (pointer >= 0 && pointer < maxentTagger.prob.lambda.length)
					lambda[numFeats] = maxentTagger.prob.lambda[pointer];
				fTagAssociations[tagIndex] = numFeats;
			}

			numFeats++;

		} // foreach FeatureKey fK
		HashSet<FeatureKey> inactiveFeatureSet = new HashSet<FeatureKey>();
		for (int i = 0, size = maxentTagger.fAssociations.size(); i < size; ++i) {
			Map<String, int[]> map = maxentTagger.fAssociations.get(i);
			for (Map.Entry<String, int[]> entry : map.entrySet()) {
				int[] pointer = entry.getValue();
				for (int tagIndex = 0; tagIndex < ySize; ++tagIndex) {
					if (pointer[tagIndex] >= 0) {
						FeatureKey key = new FeatureKey(i, entry.getKey(), maxentTagger.getTag(tagIndex));
						if (!activeFeatureSet.contains(key)) {
							inactiveFeatureSet.add(key);
						}
					}
				}
			}
		...
		//deal with the new features that aren't included in the old feature Map.
		for (FeatureKey fK : inactiveFeatureSet) {
			int[] array = maxentTagger.fAssociations.get(fK.num).get(fK.val);
			//				if (maxentTagger.fAssociations.get(fK.num).containsKey(fK.val)){
			int pointer = array[maxentTagger.getTagIndex(fK.tag)];
			if (pointer < 0)
				throw new RuntimeException("pointer < 0");
			array[maxentTagger.getTagIndex(fK.tag)] = -1;
			...
			if (indexValid < 0) {
				maxentTagger.fAssociations.get(fK.num).remove(fK.val);
			}
		}

		if (numFeats != activeFeatureSet.size())
			throw new RuntimeException("numFeats != activeFeatureSet.size()");

		log.info("total feats: " + sTemplates.size() + ", populated: " + numFeats + ", with an exctraction percentage = " + numFeats * 100.0 / sTemplates.size() + "%");

		if (lambda.length != numFeats)
			lambda = Arrays.copyOf(lambda, numFeats);
		maxentTagger.prob.lambda = lambda;
	...
	/*
	 * public String getY(int index) { return
	 * maxentTagger.tags.getTag(vArray[index][1]); }
	 */

	/*
	 * public static void main(String[] args) { int[] hPos = {0, 1, 2, -1, -2};
	 * boolean[] isTag = {false, false, false, true, true}; maxentTagger.init();
	...
	public String toString() {
		...
		str += "sTemplates = ";
		str += Utility.toString(sTemplates);

		str += Utility.lineSeparator;
		str += "tHistories = ";
		str += Utility.toString(tHistories);
		...
		str += Utility.lineSeparator;
		str += "tFeature = ";
		str += tFeature.toString();

		str += Utility.lineSeparator;
		str += "fnumArr = ";
		str += Utility.toString(fnumArr);
