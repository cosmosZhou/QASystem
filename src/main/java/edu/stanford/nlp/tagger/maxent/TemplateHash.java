...
class ListInstances {
	...
	/*
	 * Methods unused: commented for now. public void save(DataOutputStream rf)
	 * { try { rf.writeInt(v.size()); int[] arr = getInstances(); for (int i =
	 * 0; i < v.size(); i++) { rf.writeInt(arr[i]); } } catch (Exception e) {
	 * e.printStackTrace(); } }
	 * 
	 * public void read(DataInputStream rf) { try { int len = rf.readInt(); for
	 * (int i = 0; i < len; i++) { int x = rf.readInt(); add(x); } } catch
	...
	public String toString() {
		...
		str += "\t";
		str += "positions = ";
		str += Utility.toString(positions);
...
public class TemplateHash {
	...
	/*
	 * Methods unused. Commented for now. public void save(DataOutputStream rf)
	 * { try { Pair[] keys = new Pair[tempHash.keySet().size()];
	 * tempHash.keySet().toArray(keys); rf.writeInt(keys.length); for (Pair key
	 * : keys) { //rf.writeInt(s.length()); //rf.write(s.getBytes());
	 * key.save(rf); tempHash.get(key).save(rf); } // for
	 * 
	 * } catch (Exception e) { e.printStackTrace(); } }
	 * 
	 * private void read(InDataStreamFile rf) { try { int numElem =
	 * rf.readInt(); for (int i = 0; i < numElem; i++) { //int
	 * strLen=rf.readInt(); //byte[] buff=new byte[strLen]; //rf.read(buff);
	 * //String s=new String(buff); Pair<String,String> sWT =
	 * Pair.readStringPair(rf); Pair<Integer,String> wT = new
	 * Pair<Integer,String>(Integer.parseInt(sWT.first()), sWT.second());
	 * ListInstances li = new ListInstances(); li.read(rf); tempHash.put(wT,
	 * li); }// for } catch (Exception e) { e.printStackTrace(); } }
	 * 
	 * public void print() { Object[] arr = tempHash.keySet().toArray(); for
	 * (int i = 0; i < arr.length; i++) { System.out.println(arr[i]); } }
	 * 
	 * public static void main(String[] args) { TemplateHash hT = new
	 * TemplateHash(); Pair<Integer,String> p = new Pair<Integer,String>(0,
	 * "0"); ListInstances li = new ListInstances(); li.add(14);
	 * hT.tempHash.put(p, new ListInstances()); if (hT.tempHash.containsKey(p))
	...
	public String toString() {
		String str = "";
		str += Utility.toString(tempHash);
		return str;
	}

	public void sanity_check() {
		for (Map.Entry<Pair<Integer, String>, ListInstances> entry : tempHash.entrySet()) {
			//			log.info("entry = " + entry);
			ListInstances listInstances = entry.getValue();
			if (TaggerExperiments.isPopulated(entry.getKey().first, listInstances.getNum(), maxentTagger)) {
				if (listInstances.getInstances().length != listInstances.getNum()) {
					throw new RuntimeException("listInstances.getInstances().length != listInstances.getNum()");
				}
			}
			else{
				if (listInstances.getInstances().length != 0) {
					throw new RuntimeException("listInstances.getInstances().length != 0");
				}				
			}
		}
	}
